 ...
 *
 * @return An absolute path to FLAGSHIP.
 */
const getPathToFLAGSHIP = (): string => {
  if (_pathToFLAGSHIP) {
 ...
 *
 * @param paths A list of path components to resolve relative to FLAGSHIP.
 * @return A resolved path relative to FLAGSHIP.
 */
const resolvePathFromFLAGSHIP = (...paths: string[]): string =>
  path.resolve.apply(path, [getPathToFLAGSHIP(), ...paths]);
...
const getPbxprojFilePath = (configuration: Config): string =>
  resolvePathFromProject('ios', `${configuration.name}.xcodeproj`, 'project.pbxproj');
 ...
 */
const getFastfilePathIOS = (): string => resolvePathFromProject('ios', 'fastlane', 'Fastfile');
...
const getMainActivityPath = (configuration: Config): string =>
  resolvePathFromProject(getNativeProjectPathAndriod(configuration), 'MainActivity.java');
...
const getMainApplicationPath = (configuration: Config): string =>
  resolvePathFromProject(getNativeProjectPathAndriod(configuration), 'MainApplication.java');
 ...
 */
const getManifestPath = (): string => resolvePathFromProject(getMainPath(), 'AndroidManifest.xml');
 ...
 */
const getGradlePath = (): string => resolvePathFromProject('android', 'app', 'build.gradle');
 ...
 */
const getMainPath = (): string => resolvePathFromProject('android', 'app', 'src', 'main');
 ...
 * @return The path to the main directory.
 */
const getDebugPath = (): string => resolvePathFromProject('android', 'app', 'src', 'debug');
 ...
 */
const getAssetsPath = (): string => resolvePathFromProject(getMainPath(), 'assets');
 ...
 */
const getResourcesPath = (): string => resolvePathFromProject(getMainPath(), 'res');
