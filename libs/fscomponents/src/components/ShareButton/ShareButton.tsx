...
import type { ShareButtonProps } from './ShareButtonProps';
...
export const ShareButton: FunctionComponent<ShareButtonProps> = memo((props) => {
  const onSharePress = () => {
    Share.share(props.content, props.options).catch((error) => {
      ...
      if (error.message === 'Share is not supported in this browser') {
        ...
        // @ts-expect-error ShareContent has a version with url and one without
        if (props.content && props.content.url) {
          // @ts-expect-error ShareContent has a version with url and one without
          const { url } = props.content;
          // @ts-expect-error Web returns false on failure, native returns void
