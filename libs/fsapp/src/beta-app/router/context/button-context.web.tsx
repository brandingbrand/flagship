...
export interface ButtonService {
  onPress: ((buttonId: string, callback: () => void) => () => void) &
    ((buttonId: string, componentId: string, callback: () => void) => () => void);
}
...
export const useButtonEffect = (
  _callback: () => void,
  _ids: [buttonId: string, componentId: string] | [buttonId: string]
) => {
  // NOOP
};

export { Fragment as ButtonProvider } from 'react';
