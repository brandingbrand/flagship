...
export default function targetInstancesByTimeOfDay(
  ...
  const instanceEndTime =
    instance.targets && instance.targets[kTimeOfDayKey] && instance.targets[kTimeOfDayKey].time_end;
  ...
  if (instanceStartTime && instanceEndTime) {
    ...
    const instanceLocalStartTime =
      new Date().setHours(instanceStartHourAndMinute[0], instanceStartHourAndMinute[1], 0, 0) -
      currentDate.getTimezoneOffset() * 60 * 1000;

    const instanceLocalEndTime =
      new Date().setHours(instanceEndHourAndMinute[0], instanceEndHourAndMinute[1], 0, 0) -
      currentDate.getTimezoneOffset() * 60 * 1000;
  ...
  if (campaignStartTime && campaignEndTime) {
    ...
    const campaignLocalStartTime =
      new Date().setHours(campaignStartHourAndMinute[0], campaignStartHourAndMinute[1], 0, 0) -
      currentDate.getTimezoneOffset() * 60 * 1000;

    const campaignLocalEndTime =
      new Date().setHours(campaignEndHourAndMinute[0], campaignEndHourAndMinute[1], 0, 0) -
      currentDate.getTimezoneOffset() * 60 * 1000;
