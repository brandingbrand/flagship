...
`@brandingbrand/kernel-core` is a foundational package that provides modules for generating native code for a React Native app.

### Getting Started

<div class="flex w-full my-6">
  <CodeCopy args="yarn add -D @brandingbrand/kernel-core" />
</div>
...
The `tsc` executor typescript compiles the typescript envs from the `.kernelrc/env` directory. It also copies the command-line option env compiled file to an `env.js` file to be used by downstream services.
...
The `plugins` executor gathers all the plugins specified in the `package.json` under the `kernel.plugins` configuration, according to the command-line option `platform`. It then executes these plugins in order according to the priority array.
...
The `env` executor removes all typescript compiled env js files in the `.kernelrc/env` directory.
...
[**dfcefeb9d**](https://github.com/brandingbrand/flagship/pull/2508/commits/dfcefeb9d32eabe878bfd0a9ae0729c441d8d6c5) -- disable dev menu when cli option `--release` is `true`.
