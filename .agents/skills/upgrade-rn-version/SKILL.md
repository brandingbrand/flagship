...
8. **Changeset.** One file, message `add support for react-native 0.XX`, minor bump on the packages an adoption touches: `code-plugin-verify-dependencies`, `code-templates`, `code-cli-kit`, `code-cli`. Check that list against what the previous adoption bumped with `grep -rl "add support for react-native 0.YY" packages/*/CHANGELOG.md`, where `0.YY` is the minor below the one you are adopting; follow the changelogs if they disagree with the list. Do not look for the previous minor's changeset in `.changeset/`, which holds only unreleased ones: it is consumed when the release publishes. Declare another package only if this adoption actually modified it; do not declare one because it seems related.
