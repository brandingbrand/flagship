...
You can configure the `hiddenEnvs` property within each build config via the `code-plugin-env` plugin. This allows you to exclude certain environments from the resulting app bundle, which is useful for preventing production environments from use in internal builds.

```typescript title="build.internal.ts" ins={4,12-16}
