...
The `@brandingbrand/code-app-env/ui` sub-module also provides pre-built UI components that match the style of the default `DevMenu` screens.

```tsx title="DebugScreen.tsx"
import React from "react";
import { View } from "react-native";
import { defineDevMenuScreen } from "@brandingbrand/code-app-env";
import { Text, Button } from "@brandingbrand/code-app-env/ui";

export const DebugScreen = defineDevMenuScreen('DebugScreen', () => (
  <View>
    <Text type="titleSm">Debug Screen</Text>
    <Button onPress={() => console.log('Debug button pressed')}>
      Log Message
    </Button>
  </View>
));
