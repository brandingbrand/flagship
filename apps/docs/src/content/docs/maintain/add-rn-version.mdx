...
3. Update the `devDependencies` of the `code-app-env` package.

    Due to the way the example app is included within the monorepo, the `devDependencies` of the `code-app-env`'s `package.json` must be manually updated to reflect the new React Native version. Failure to do so will lead to duplicate installations of `react` and `react-native`, which will cause runtime errors.

    In `./packages/app-env/package.json`, update the version range of `react` and `react-native` within the `devDependencies` section to match the same version ranges used in the example app. leave `peerDependencies` unchanged.
