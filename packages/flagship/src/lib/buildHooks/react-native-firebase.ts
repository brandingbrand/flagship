...
const buildHooks: BuildHook[] = [
  {
    ...
    script: configuration => {
      ...
      const jsonFilePath =
        path.project.resolve(configuration.firebase.android.googleServicesJsonFile);
      ...
      // and replace compile command with implementation
    ...
    implementation 'com.google.firebase:firebase-core:16.0.1'
      ...
      let gradleBuild = fs.readFileSync(
        path.resolve('android', 'build.gradle'), { encoding: 'utf8' });
  ...
  },
  {
    ...
    script: configuration => {
      if (!(configuration.firebase
        ...
        // Firebase includes GoogleAppMeasurement as a dependency automatically, but the latest
        // version 5.4.0 was compiled with Xcode 10 which is not yet supported by Flagship
