# @brandingbrand/code-preset-react-native

## 1.3.0

### Minor Changes

- add React Native 0.83 support

### Patch Changes

- Updated dependencies [45b584c]
- Updated dependencies
- Updated dependencies [6d74f27]
  - @brandingbrand/code-plugin-verify-dependencies@1.3.0
  - @brandingbrand/code-plugin-transform-template@1.3.0

## 1.2.0

### Minor Changes

- a4d0c66: Add Network Security Config plugin
- 83646f8: React Native 0.82 Support
- c5f263b: React Native 0.81 Support
- 746cf0d: Add preset-specific build config types

### Patch Changes

- updated to support react-native-navigation 0.77 and 0.78
- Updated dependencies [8bb3b38]
- Updated dependencies [39e4507]
- Updated dependencies [2fc57cc]
- Updated dependencies [a7826f5]
- Updated dependencies [83646f8]
- Updated dependencies [c5f263b]
  - @brandingbrand/code-plugin-network-security-config@1.0.0
  - @brandingbrand/code-plugin-env@1.2.0
  - @brandingbrand/code-plugin-verify-dependencies@1.2.0
  - @brandingbrand/code-plugin-transform-template@1.2.0

## 1.1.0

### Minor Changes

- 855c897: reorder env plugin to be executed after transform-template as it may rely on template files

## 1.0.2

### Patch Changes

- fix: update code-plugin-transform-template dependency
- Updated dependencies
  - @brandingbrand/code-plugin-transform-template@1.0.2

## 1.0.1

### Patch Changes

- Updated dependencies
  - @brandingbrand/code-plugin-transform-template@^1.0.1

## 1.0.0

### Major Changes

- react native <0.79 support

### Patch Changes

- Updated dependencies
  - @brandingbrand/code-plugin-verify-dependencies@1.0.0
  - @brandingbrand/code-plugin-transform-template@1.0.0
  - @brandingbrand/code-plugin-packager-install@1.0.0
  - @brandingbrand/code-plugin-clean@1.0.0
  - @brandingbrand/code-plugin-env@1.0.0
