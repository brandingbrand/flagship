 ...
 * ```
 */
export const DevMenuContext = React.createContext<DevMenuContextType | null>(
  null,
);
 ...
 *   const [activeScreen, setScreen] = useScreen();
 *
 *   return (
 *     <div>
 *       <button onClick={() => setScreen({ Component: HomeScreen })}>Home</button>
 *       <button onClick={() => setScreen({ Component: Profile })}>Profile</button>
 ...
 * ```
 */
export const [useScreen, ScreenContextProvider] = createStateContext<{
  Component: React.ComponentType;
} | null>(null);

/**
 * A custom hook and provider for managing the selected environment within the EnvSwitcher.
 */
export const [useEnvSwitcher, EnvSwitcherProvider] =
  createStateContext<string>(envName);
