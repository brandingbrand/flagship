...
import {extractEnvName, getEnvFileList, loadFlagshipAppEnvRC} from '../shared';
...
export default function ({types: t}: typeof Babel): Babel.PluginObj {
  ...
  function getEnvs() {
    ...
    const envFiles = getEnvFileList(config);
     ...
     */
    const envs = envFiles.reduce<Record<string, any>>((acc, curr) => {
      ...
      const envName = extractEnvName(curr);
      if (envName) {
        acc[envName] = env;
      }
