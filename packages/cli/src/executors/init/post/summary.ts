...
import { exec, fs, path, summary } from "@brandingbrand/code-core";

import type { Config, Options, Summary } from "@brandingbrand/code-core";
...
export const execute = async (options: Options.InitOptions, config: Config) => {
  ...
  summary.items.forEach((it) => {
    const row = Object.keys(it)
      ...
      .map((el) => {
        if (el === "error" && it[el as keyof Summary.Items]) errors++;

        if (el === "warning" && it[el as keyof Summary.Items]) warnings++;

        if (el === "success" || el === "error" || el === "warning") {
          return it[el as keyof Summary.Items] ? "✓" : "";
        }
