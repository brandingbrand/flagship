import {canRunAndroid, canRunIOS, logger} from '@brandingbrand/code-cli-kit';
...
export default defineAction(async () => {
  ...
  if (canRunAndroid(config.options) && canRunIOS(config.options)) {
    ...
    logger.log(
      'transformed ios and android projects based upon build configuration',
    );
  ...
  if (canRunAndroid(config.options) && !canRunIOS(config.options)) {
    ...
    logger.log('transformed android project based upon build configuration');
