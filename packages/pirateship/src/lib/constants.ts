export const UPDATE_WISHLIST = 'UPDATE_WISHLIST';
export const SIGN_IN = 'SIGN_IN';
export const UPDATE_ACCOUNT = 'UPDATE_ACCOUNT';
export const SIGN_OUT = 'SIGN_OUT';
export const SET_WISHLIST_UPDATING = 'SET_WISHLIST_UPDATING';
export const RESET_WISHLIST_UPDATING = 'RESET_WISHLIST_UPDATING';
export const CUSTOMER_SERVICE_PHONE_NUMBER = '1-800-000-0000';
export const UPDATE_CART = 'UPDATE_CART';
export const SET_CART_UPDATING = 'SET_CART_UPDATING';
export const RESET_CART_UPDATING = 'RESET_CART_UPDATING';
export const UPDATE_TOP_CATEGORIES = 'UPDATE_TOP_CATEGORIES';
export const UPDATE_PROMO_PRODUCTS = 'UPDATE_PROMO_PRODUCTS';
export const LOAD_RECENTLY_VIEWED = 'LOAD_RECENTLY_VIEWED';
export const UPDATE_RECENTLY_VIEWED = 'UPDATE_RECENTLY_VIEWED';
export const EMAIL_REGEX = /[a-z0-9!#$%&'*+/=?^_`{|}~.-]+@[a-z0-9-]+(\.[a-z0-9-]+)*/;
export const STORAGE_KEYS = {
  username: 'signin_email',
  password: 'signin_password'
};
